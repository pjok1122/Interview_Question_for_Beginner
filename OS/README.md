...
프로세스는 실행 중인 프로그램을 의미한다. 프로세스의 구조는 위의 그림처럼 각각의 주소공간을 할당받는다.

- Code 영역 : 프로그램 명령 코드가 저장되는 메모리 영역 (같은 프로그램 내에서는 따로 관리하여 공유한다.)
- Data 영역 : 전역변수, 정적변수, 배열 등 (초기화된 데이터)
- Heap 영역 : 동적 할당 시 사용(new(), malloc() 등)
- Stack 영역 : 지역변수, 매개변수, 리턴 값 (임시 메모리 영역)

프로세스 간에는 데이터를 공유하기 위해서 Pipe, 공유 메모리 등을 사용해야 한다.

반면에 쓰레드 간에는 Code, Data, Heap 영역을 공유하여 사용할 수 있다.

### 스레드(Thread)

스레드는 **프로세스의 실행 단위**라고 할 수 있다. 한 프로세스 내에서 동작되는 여러 실행 흐름으로 프로세스 내의 주소 공간이나 자원을 공유할 수 있다.
스레드는 스레드 ID, 프로그램 카운터, 레지스터 집합, 그리고 스택으로 구성된다. 같은 프로세스에 속한 다른 스레드와 코드, 데이터 섹션, 그리고 열린 파일이나 신호와 같은 운영체제 자원들을 공유한다.

#### 스택을 스레드마다 독립적으로 할당하는 이유

스레드는 독립적인 실행흐름을 가져야 하므로, 스택을 독립적으로 할당받아야 한다. 스택은 함수의 실행 후 되돌아올 주소 등을 저장하기 때문에 스택을 공유하게 되면, 독립적인 실행흐름을 가질 수 없다.
...
- 프로세스와 관련된 정보를 Linked List 형태로 저장하고 있다.
- PCB는 PC, 프로세스의 상태 등을 저장하여 문맥 교환 시에 반영한다.
...
- 자원을 공유하기 때문에 스레드 간의 통신이 간단하다.
- Context switching(문맥교환)에 대한 오버헤드가 프로세스에 비해 적다.
...
- 쓰레드 간의 동기화 작업이 필요하다.
- 하나의 쓰레드가 자원을 망가뜨릴 경우, 모든 쓰레드가 작동하지 못한다.
...
- 비선점형(Non-Preemptive) 스케줄링 (작업이 끝날 때까지 CPU를 양보하지 않음)
...
- starvation
- 프로세스의 Remaining time을 예측하는 별도의 알고리즘이 필요하다.
...
동기는 함수를 실행하면 결과를 리턴할 때까지 `blocking` 되고, 비동기 함수는 `blocking` 되지 않고 이벤트 큐에 넣거나 백그라운드 스레드에게 해당 task를 위임하고 바로 다음 코드를 실행한다.
...
공유 자원에 접근하는 코드 영역을 Critical Section 이라 칭한다.
...
- 프로세스 A와 B가 서로가 쥐고 있는 자원을 요구할 때 교착상태가 발생한다.
...
메모리의 관리를 위해 사용되는 기법. 다중 프로그래밍 환경에서 프로세스의 메모리를 보조 기억장치(e.g. 하드디스크)로 내보내고 다른 프로세스의 메모리를 불러 들일 수 있다.
...
압축 : 외부 단편화를 해소하기 위해 프로세스가 사용하는 공간들을 한쪽으로 몰아, 자유공간을 확보하는 방법론 이지만, 작업효율이 좋지 않다. (디스크 조각모음)
...
![Segmentation](./images/세그멘테이션.jpg)

프로세스를 서로 다른 크기를 가진 논리적 블록으로 분리하여 물리적 공간에 할당하는 메모리 관리 기법이다.
...
> 가상 메모리는 서로 다른 프로세스가 페이지를 공유할 수 있다.
...
> 초기에 필요한 페이지만 물리 메모리에 올리고, 추가 요구 페이지가 있을 때마다 페이지를 물리 메모리에 올린다.
...
이 알고리즘의 핵심은 `앞으로 가장 오랫동안 사용되지 않을 페이지를 찾아 교체`하는 것이다. 실현 가능성이 없는 알고리즘.
...
> 프로그램은 최근에 참조된 내용을 다시 참조하거나, 인접한 주소의 내용을 참조하는 성향이 있다. 이를 지역성이라고 한다.
